...
const FieldFarmContainer = styled.div`
  form {
    display: flex;
...
const FieldForm: React.FC<FieldFormProps> = ({ onSubmit }) => {
  ...
  const handleChange = (
    ...
    switch (name) {
      case "name":
        ...
        case "fieldArea":
        setFieldArea(
          typeof numericValue === "number" ? numericValue : parseFloat(value)
        );
        break;
      case "soilId":
        setSoilId(value);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!fieldData.name || !fieldData.fieldArea || !fieldData.soilId || !fieldData.farmId) {
